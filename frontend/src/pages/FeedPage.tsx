...
export function FeedPage({ profile, onSelectTopic }: Props) {
  ...
  const generateFreshDeck = async () => {
    ...
    try {
      ...
      const items = await geminiService.fetchTopics(profile.categories, history, profile.preferredModel || 'gemini-1.5-flash');
      const now = new Date();
  ...
  const handleWildcard = async () => {
    ...
    try {
      const wildcardItem = await geminiService.fetchWildcard(profile.categories, profile.preferredModel || 'gemini-1.5-flash');
      const now = new Date();
