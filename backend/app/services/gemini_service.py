...
class GeminiService:
    ...
    async def generate_topics(self, categories: list[str], read_history: list[str] = None, model: str = "gemini-1.5-flash", api_key: str = None) -> list[dict]:
        ...
        prompt = f"""You are a micro-learning topic curator for AntiScroll.
User categories: {cats_str}
...
4. Include concrete examples or code snippets if relevant
...
{{
  "type": "wordle",
  "data": {{ "targetWord": "RAFT", "hint": "Consensus algorithm keyword", "maxAttempts": 6 }}
}}
```
Flashcard:
```game-json
{{
  "type": "flashcard",
  "data": {{ "cards": [ {{"front": "Question 1", "back": "Answer 1"}} ] }}
}}
```
Concept Match:
```game-json
{{
  "type": "concept_match",
  "data": {{ "pairs": [ {{"term": "Term 1", "definition": "Def 1"}} ] }}
}}
```
Crossword:
```game-json
{{
  ...
  "data": {{
    ...
    "clues": [
      {{ "number": 1, "direction": "across", "clue": "State of consensus", "answer": "RAFT", "startRow": 0, "startCol": 0 }}
    ]
  }}
}}
```
